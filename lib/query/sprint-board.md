...
Callers use `useSprintBoard` and `createSprintBoardPersistAdapter` from `lib/query/useSprintBoard.ts`. The HTTP fetcher is `getDashboardData` in `lib/services/client/getDashboardData`. The server loader is `loadSprintBoard` in `lib/services/server/sprintBoard`, which calls `lib/services/server/jira`. Params, function type, and `SprintBoardData` live in `lib/services/shared/getDashboardData`. Domain types (`SprintRef`, `LedgerParent`) live in `lib/services/shared/jira`.
